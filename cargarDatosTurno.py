...
def cargar_dim_turno_1x1(conn, cursor, df_origen):
    """Inserta nuevos valores únicos de turno uno por uno."""
    
    tabla_dim = 'Dim_Turno'
    col_csv = 'Turno'
    col_dim = 'nombre_turno' 
    ...
    if not valores_a_insertar:
        print(f"-> {tabla_dim}: No hay turnos nuevos que insertar.")
        return

    print(f"-> {tabla_dim}: Insertando {len(valores_a_insertar)} turnos nuevos (1x1)...")
...
cargar_dim_turno_1x1(conn, cursor, df_limpio)

cursor.close()
conn.close()
print("Proceso de carga de Dim_Turno finalizado.")